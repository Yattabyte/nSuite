...
Installer::Installer(const HINSTANCE hInstance) : Installer()
{
	...
	if (!m_archive.exists())
		yatta::Log::PushText("Critical failure: archive doesn't exist!\r\n");
	else {
		// Read in header
		yatta::Directory::PackageHeader packageHeader;
		std::byte* dataPtr(nullptr);
		size_t dataSize(0ULL);
		yatta::Buffer(m_archive.getPtr(), m_archive.getSize(), false).readHeader(&packageHeader, &dataPtr, dataSize);

		// Ensure header title matches
		if (!packageHeader.isValid())
			yatta::Log::PushText("Critical failure: cannot parse packaged content's header!\r\n");
		else {
			m_packageName = packageHeader.m_folderName;
			yatta::Buffer::CompressionHeader header;
			yatta::Buffer(dataPtr, dataSize, false).readHeader(&header, &dataPtr, dataSize);

			// Ensure header title matches
			if (!header.isValid())
				yatta::Log::PushText("Critical failure: cannot parse archive's packaged content's header!\r\n");
			else {
				...
				if (m_mfStrings[L"name"].empty() && !m_packageName.empty())
					m_mfStrings[L"name"] = yatta::to_wideString(m_packageName);
...
void Installer::beginInstallation()
{
	m_threader.addJob([&]() {
		...
		yatta::Resource unInstaller(IDR_UNInstallER, "UNInstallER");
		yatta::Resource manifest(IDR_MANIFEST, "MANIFEST");
		if (!unInstaller.exists()) {
			...
			const auto directory = yatta::Directory::SanitizePath(getDirectory());
			const auto virtual_directory = yatta::Directory(yatta::Buffer(m_archive.getPtr(), m_archive.getSize(), false), directory);
			if (!virtual_directory.apply_folder())
				...
				if (!file.is_open()) {
					yatta::Log::PushText("Cannot write unInstaller to disk, aborting...\r\n");
					invalidate();
				}
				yatta::Log::PushText("Uninstaller: \"" + unInstallerPath + "\"\r\n");
				...
				if (!(bool)UpdateResource(handle, "MANIFEST", MAKEINTRESOURCE(IDR_MANIFEST), MAKELANGID(LANG_NEUTRAL, SUBLANG_DEFAULT), (LPVOID)newManifest.c_str(), (DWORD)newManifest.size())) {
					yatta::Log::PushText("Cannot write manifest contents to the unInstaller, aborting...\r\n");
					invalidate();
				}
				...
				if (RegCreateKeyExW(HKEY_LOCAL_MACHINE, (L"SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\Uninstall\\" + m_mfStrings[L"name"]).c_str(), 0, nullptr, REG_OPTION_NON_VOLATILE, KEY_ALL_ACCESS, nullptr, &hkey, nullptr) == ERROR_SUCCESS) {
					auto
						name = yatta::from_wideString(m_mfStrings[L"name"]);
					auto
						version = yatta::from_wideString(m_mfStrings[L"version"]);
					auto
						publisher = yatta::from_wideString(m_mfStrings[L"publisher"]);
					auto
						icon = yatta::from_wideString(m_mfStrings[L"icon"]);
					DWORD ONE = 1;
